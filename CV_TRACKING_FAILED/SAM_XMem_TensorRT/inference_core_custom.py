...
class InferenceCore:
    ...
    def step(self, image, mask=None, valid_labels=None, end=False):
        ...
        if is_mem_frame:
            # === Occlusion Handling ===
            # If the object is occluded (mask area is too small), we should NOT update the memory.
            # This prevents "polluting" the memory with background features.
            # We check the area of the predicted mask (excluding background).
            
            should_update = True
            if pred_prob_with_bg is not None:
                # pred_prob_with_bg shape: (K+1, H, W)
                # We check if ANY object has a significant presence
                obj_probs = pred_prob_with_bg[1:] # (K, H, W)
                
                # Calculate area for each object
                obj_areas = (obj_probs > 0.5).sum(dim=(1, 2)) # (K,)
                
                # If the largest object is smaller than a threshold (e.g., 100 pixels), we consider it occluded
                # You might want to adjust this threshold based on video resolution
                if obj_areas.max() < 100:
                    should_update = False
                    # print(f"Frame {self.curr_ti}: Occlusion detected (area={obj_areas.max()}), skipping memory update.")
