...
def process_video(video_path, x1, y1, x2, y2):
    ...
    stats_file_path = OUTPUT_DIR / f"{video_stem}.txt"
    try:
...
with gr.Blocks(title="🎯 零样本目标跟踪器", theme=gr.themes.Soft()) as demo:
    ...
    **使用说明:**
    ...
    ---
    ### 🔧 技术栈:
    - **主跟踪器**: OSTrack (One-Stream Transformer, ViT-Base)
    - **架构**: 单流 Transformer，联合特征提取与关系建模
    - **恢复机制**: 全局搜索 (Global Search / Sliding Window)
    - **加速**: CUDA (PyTorch)
    
    ### 📈 性能优化 (Ferrari Edition):
    - 状态 TRACKING (>0.5): OSTrack 高速跟踪
    - 状态 LOW_CONF (0.3-0.5): 扩大搜索区域
    - 状态 LOST (<0.3): 触发全图滑动窗口搜索 (Global Search)
    - 彻底移除 YOLO，实现纯视觉单模型跟踪
    """)


if __name__ == "__main__":
    demo.launch(share=False, server_name="0.0.0.0", server_port=8666)
